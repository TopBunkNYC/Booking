...
class GuestItem extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      a: this.props.person === "Adults" ? 1 : 0,
      minus: true,
      plus: false,
  ...
  componentDidUpdate() {
    ...
    if (
      this.props.person === "Adults" &&
      this.state.a === 1 &&
      !this.state.minus
    ) {
      //overriding Adults minus functionalities to stop when = 1
      console.log("turnOff");
      this.setState({
        minus: true,
        colorSub: "ButtonASOff"
  ...
  render() {
    return (
      <Row className="GuestRow">
        ...
        <Col xsOffset={1} xs={1}>
          <button
            className={`${this.state.colorPlus}`}
